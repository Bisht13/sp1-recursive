...
pub fn main() {
    ...
    let vkey_hash =
        Fr::from_slice(&sp1_zkvm::io::read_vec()[8..]).expect("Unable to read vkey_hash");
    let committed_values_digest = Fr::from_slice(&sp1_zkvm::io::read_vec()[8..])
        .expect("Unabel to read committed_values_digest");
